...
def agg_event_role_tpfpfn_stats(pred_record, gold_record, role_num):
    """
    Aggregate TP,FP,FN statistics for a single event prediction of one instance.
    A pred_record should be formated as
    [((Role Index)
        argument 1, ...
     ), ...
...
def transfer_output_to_eval(output_data, event_role_dict):
    """
    将预测结果转化成评估时规定格式的输入
    [(Sample Index)
        [(Event Index)
    ...
    for sample in output_data:
        event_trans = []
        event_list = {event['event_type']: {arg['role']: arg['argument'] for arg in event['arguments']}for event in sample['event_list']}
        for evt_type, role_type_list in event_role_dict:
            if evt_type in event_list:
                roles = event_list[evt_type]
                evt_tmp = []
                for role_type in role_type_list:
                    if role_type in roles:
                        evt_tmp.append(roles[role_type])
                    else:
                        evt_tmp.append(None)
                event_trans.append(evt_tmp)
            else:
                event_trans.append(None)
        eval_data.append(event_trans)
    return eval_data
